...
<?php
$con=mysqli_connect("localhost","root","Suraj@123","organic_shop_db")or die ("Couldnt connect");

$disp="SELECT  * from tbl_product_category ORDER BY prod_category_name ASC";

$disp_sub="SELECT  * from tbl_prod_subcatg ";
$disp_result=mysqli_query($con,$disp);
$disp_sub_result=mysqli_query($con,$disp_sub);
$prodname="";
...
if ($flag == 0) {
    ...
    if (mysqli_query($con, $q_ins) === TRUE) {
        $last_inserted_id = mysqli_insert_id($con); // Get the last inserted ID
        
        $subcatg_name = "your_subcatg_name"; // Replace with your subcatg_name
        $status = "your_status"; // Replace with your status

        $q_ins_subcatg = "INSERT INTO tbl_prod_subcatg (product_category_id, subcatg_name, status) VALUES ('$last_inserted_id', '$subcatg_name', '$status')";

        if (mysqli_query($con, $q_ins_subcatg) === TRUE) {
