...
$disp_prod_type="Select * from tbl_product_category WHERE status='VALID'";

$d_prod_type=mysqli_query($con,$disp_prod_type);


$disp_prod_cat="Select * from tbl_product_category WHERE status='VALID'";

$d_prod_cat=mysqli_query($con,$disp_prod_cat);
...
<body> 
	...
	<div class="container">
		...
		<table id="designs">
			<form action="add_sub_cat_action.php" method="POST" name="add_sub_cat_action.php" id="add_sub_cat_action.php">
		...
		<label >Select product category </label>
													...
													<select name="category" id="category" class="category" class="form-control" required>
													
														<option value="" required>--Select Category--</option>
													   <?php
																while($catr=mysqli_fetch_array($d_prod_cat))
														{
														?>

													  <option value="<?php echo $catr['product_category_id']; ?>"><?php echo $catr['prod_category_name']; ?></option>
															<?php
															}
															?>
													</select>
													...
													<label >Existing subcategories for the selected product category</label>
													<!--<select  name="subcat" id="subcat" class="subcat">
													<option value="" required>--Select product Category to view existing sub categories--</option>
													</select>
													-->
													<textarea  name="subcat" id="subcat" class="subcat" rows="5"style="width:100%" readonly> </textarea>
													</br>
													</br>
...
<p>Select Sub category of this product</p>
													<select  name="subcat" id="subcat"   class="form-control"  class="subcat" required>
													<option value="">--Select Category before select sub category--</option>
...
$(".subcat").html(catg);
		...
		<tr>
			...
			<input type="text" name="sub_cat_name" id="sub_cat_name" autocomplete="off" onblur="checkNewProdCat()" placeholder="new sub category" value="" required>
				...
				<script>			
							function checkNewProdCat()
									...
									var fnamevalue=document.getElementById("sub_cat_name").value;
									//document.getElementById('errorname').innerHTML=fnamevalue;
									
									if (fnamevalue==null || add_sub_cat_action.php.sub_cat_name.value.length==0)
									{
									document.getElementById('nerrorname').innerHTML="Mandatory Field!";
	
									}
									else if(add_new_prod_cat.sub_cat_name.value.match(letters))
									{
...
&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
						...
						<center><input type="submit" name="create" id="create" value="Add product category">	</center>		
						</td>
...
<h2 font size="4"><i> Product SubCategory</i> </h2>


<table>
  <tr>
    <th>NO </th>
    <th>PRODUCT SUBCATEGORYS</th>
    <th>EDIT</th>
	<th>DELETE<th>
  </tr>
<tbody>
...
`							   <?php
									  $checknamep="Select * from tbl_prod_subcatg";
                                          $disp_presult=mysqli_query($con,$checknamep);
										  $counter = 0;
while($row=mysqli_fetch_array($disp_presult))
                                          {
                                            ...
                                            <tr>
                                                ...
                                                <td>  <?php echo $row["subcatg_name"];?></td>
                                                <td class="text-right"><a style="color:#FF0000;"href="docedit.php?k=<?php echo $row['prod_subcatg_id']?>& j=<?php echo $row['subcatg_name']?>"><img src="images/edit.png" width="25" height="25"></a></td>
                                                <td class="text-right"><a style="color:#FF0000;"  href="dodelete.php?j=<?php echo $row['prod_subcatg_id']?>"><img src="images/delete.jfif" width="25" height="25"></a></td>
                                            </tr>
